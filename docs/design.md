...
- Tool names use clear action-based verbs (e.g., `read_text_file`, `write_file`, `search_files`, `list_directory`) that describe exactly what the tool does and follow a consistent snake_case naming style.

- Tool descriptions are short and focused, usually one sentence explaining the tool's purpose without implementation details. Additional behavior and constraints are documented separately.

- Inputs are clearly documented with their data types, optional parameters, and expected behavior, making it easy to understand how each tool should be called.

- Error conditions and limitations are described in a direct and informative way. Examples include messages such as "Cannot specify both head and tail simultaneously" and "Fails if destination exists," explaining exactly why an operation would be rejected.

- Each tool has a single, well-defined responsibility. Instead of combining multiple operations into one tool, related functionality is split into separate tools such as `read_text_file`, `edit_file`, `move_file`, and `get_file_info`.